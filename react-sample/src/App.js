...
import { BrowserRouter as Router, Switch, Route, Link } from "react-router-dom"; //react-routerで使う
import {
  countPlus,
  countMinus,
  newTask,
          ...
          <table>
            ...
            {props.tickets.map((ticket, index) => (
              <tr id={index}>
                ...
                <td>
                  <button onClick={() => props.removeTicket(index)}>
                    {" "}
                    削除{" "}
                  </button>
                </td>
    ...
    return (
      <React.Fragment>
        <Router>
          <div>
            <button>
              <Link to="/count">Countする</Link>
            </button>
            <button>
              <Link to="/todo2">TODOリスト(関数＆redux)を表示する</Link>
            </button>
            <button>
              <Link to="/todo1">TODOリスト(クラス＆redux)を表示する</Link>
            </button>
            <button>
              <Link to="/todoview">
                【チケットTODO一覧】を表示する(react-router）
              </Link>
            </button>
            <p>
              --------------------------------------------------------------------
